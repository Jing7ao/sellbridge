...
import { getAllInventory } from "../../../src/engine/inventory-manager";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const rl = checkRateLimit(`inventory:${ip}`, { maxRequests: 20 });
    if (!rl.allowed) {
    ...
    log.info("Inventory fetched", { userId: auth.userId, total: data.stats.totalProducts });

    return NextResponse.json(data);
  } catch (err) {
    log.error("Inventory API error", { error: String(err) });
